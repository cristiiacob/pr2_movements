...
public:
	...
	Gripper()
	{
		...
		// and tell the action to spin a thread by default
		gripper_client_ = new GripperClient("r_gripper_controller/gripper_action", true);

		// wait for the gripper action server to compe up
...
// gripper_client_->sendGoalAndWait(open);
...
{
	ros::init(argc, argv, "r_gripper_driver");
